...
struct segmentTree{
    ...
    void kOne(int k, int x, int lx, int rx, int have, int &ans){
        if(~ans || have > k) return; 
        if(rx - lx == 1) {
            if(sums[x] && have + 1 == k) ans = lx;
            return;
        }
        int mid = (rx + lx) / 2;
        // if(have + sums[x] > k){
        ...
        // }
        // else if(have + sums[x] == k){

        // }
        // else if( have + sums[x] < k){
            
        // }
        if(have + sums[2 * x + 1] >= k){
            kOne(k, 2 * x + 1, lx, mid, have, ans);
        }
        else{
            kOne(k, 2 * x + 2, mid, rx, have + sums[2 * x + 1], ans);

        }
    }
    int kOne(int k){
        int ans = -1;
        kOne(k+1, 0, 0, size, 0, ans);
        return ans;
    }
};
