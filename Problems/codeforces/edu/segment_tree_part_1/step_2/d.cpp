...
struct segmentTree{
    ...
    void mx(int v, int x, int lx, int rx, int mn, int &ans){
        if(rx <= mn || ~ans || values[x] < v) return;
        if(rx - lx == 1){
        ...
        int mid = (rx + lx) / 2;
        if(values[2 * x + 1] >= v){
            mx(v, 2 * x + 1, lx, mid, mn, ans);
        }
        if(ans == -1 && values[2 * x + 2] >= v){
            mx(v, 2 * x + 2, mid, rx, mn, ans);
        }

    }

    long long mx(int v, int mn){
        int ans = -1;
        mx(v, 0, 0, size, mn, ans);
        return ans;
    }
};
